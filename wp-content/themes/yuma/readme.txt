=== Yuma ===

Contributors: sharkthemes
Tags: translation-ready, custom-header, custom-background, theme-options, custom-menu, threaded-comments, featured-images, footer-widgets, left-sidebar, right-sidebar, full-width-template, two-columns, three-columns, four-columns, grid-layout, custom-logo, blog, portfolio, photography
Requires PHP: 5.6
Requires at least: 5.0
Tested up to: 6.3
Stable tag: 1.2.1
License: GNU General Public License v3 or later
License URI: http://www.gnu.org/licenses/gpl-3.0.html

== Description ==

Explore the unlimited possibilities. Yuma is a highly dynamic and extremely flexible Gutenberg-ready WordPress theme. It includes prebuilt Patterns, which allows you to quickly add designed elements to your post or page layouts. It supports Elementor, which is the most popular page builder plugin for WordPress. It is compatible with WooCommerce, which is the best e-commerce plugin for WordPress. This fully featured theme is best for blogging sites. Since it has a wide scope of flexibility, you can create corporate, news, magazine, personal, educational, architectural, musical, and various multipurpose sites with ease. You can create unlimited header and footer designs. You can fully customize the Blog Page, Inner Pages, Typography, Theme Color, Page Layouts, and many more features via Customizer API. Yuma will make your WordPress look beautiful on all devices. It comes with cross-browser compatibility, a mobile-friendly design, and responsive features. Set up your site intuitively and without any coding skills at all.

== Installation ==

1. In your admin panel, go to Appearance > Themes and click the Add New button.
2. Click Upload Theme and Choose File, then select the theme's .zip file. Click Install Now.
3. Click Activate to use your new theme right away.

== Copyright ==

Yuma WordPress Theme, Copyright 2022, Shark Themes
Link: https://sharkthemes.com/

HTML5-Shiv is Licensed under the MIT and GPL licenses
    https://code.google.com/p/html5shiv/

Chosen Version - 1.4.2
	Source: https://github.com/harvesthq/chosen
	License: Copyright (c) 2011-2015 Harvest http://getharvest.com
	license Url : https://github.com/harvesthq/chosen/blob/master/LICENSE.md

Slick Version - 1.8.0
	Source: http://github.com/kenwheeler/slick
	License: The MIT License (MIT) copyright (c) 2013-2016
	license Url : https://github.com/kenwheeler/slick/blob/master/LICENSE

Isotope - 3.0.6
	source : https://isotope.metafizzy.co
	License: Isotope is licensed under GPLv3 for open source use
	Licenser Url: https://isotope.metafizzy.co/license.html

Packery - 2.0.1
	source : https://packery.metafizzy.co/
	License: Packery is licensed under GPLv3 for open source use
	Licenser Url: https://packery.metafizzy.co/license.html

== Screenshot Image ==
Copyright Jerry Kiesewetter
License: Creative Commons CC0 license.
License URI: https://stocksnap.io/license
Source: https://stocksnap.io/photo/abstract-flowing-PGVZUJ4RQK
Source: https://pxhere.com/en/photo/869393

Pattern Images
source: https://stocksnap.io/photo/beauty-skincare-K1I6IIHM5M
source: https://stocksnap.io/photo/woman-skin-SAA5BN6KZB
source: https://stocksnap.io/photo/beauty-woman-DRZLSHMWEA
source: https://stocksnap.io/photo/working-home-O8EAEFRYBP
source: https://stocksnap.io/photo/woman-business-YEPIEWYHTR
source: https://stocksnap.io/photo/business-man-H8PHYPQ7O8
source: https://stocksnap.io/photo/woman-skin-GX1LY6KOEB
source: https://stocksnap.io/photo/beauty-skincare-MLJXK2FAUO
source: https://stocksnap.io/photo/smiling-woman-QEFMGVEVEM
source: https://stocksnap.io/photo/woman-bathtub-1JUPEERTF9

== Frequently Asked Questions ==

= Does this theme support any plugins? =

Yuma includes support for Yuma Companion to add theme's info page, demo import and include extra features. 

Yuma includes support for demo import in Advanced Import.

Yuma includes support for theme details and showcase of prebuilt demos in Yuma Companion.

== Changelog ==

= 1.2.1 - Sep 13 2023 =
* Accessibility update

= 1.2.0 - Sep 13 2023 =
* Minor design update
* Accessibility update

= 1.1.9 - Sep 01 2023 =
* Minor design update
* Compatibility check

= 1.1.8 - Aug 06 2023 =
* Added boxed & underline hover option to header menu
* Minor design update

= 1.1.7 - Jun 21 2023 =
* Added option to remove taxonomy prefix in archive page
* Minor design update

= 1.1.6 - Apr 24 2023 =
* Minor accessibility update

= 1.1.5 - Apr 18 2023 =
* Minor design update

= 1.1.4 - Apr 12 2023 =
* Resolved text domain issue

= 1.1.3 - Apr 05 2023 =
* Resolved fadein issue for sticky header

= 1.1.2 - Apr 02 2023 =
* Minor design update
* WooCommerce full compatibility update
* Compatibility check

= 1.1.1 - Mar 27 2023 =
* Minor design update
* Resolved woocommerce cart details hide option issue

= 1.1.0 - Feb 13 2023 =
* Minor design update

= 1.0.9 - Feb 03 2023 =
* Minor design update

= 1.0.8 - Jan 23 2023 =
* Minor design update

= 1.0.7 - Dec 11 2022 =
* Added preset gredient palettes for blocks

= 1.0.6 - Dec 07 2022 =
* Minor design update
* Minor design update in blocks

= 1.0.5 - Nov 14 2022 =
* Minor design update

= 1.0.4 - Nov 11 2022 =
* Minor design update

= 1.0.3 - Nov 07 2022 =
* Minor design update

= 1.0.2 - Nov 07 2022 =
* Compatibility check
* Minor update in typography

= 1.0.1 - Oct 31 2022 =
* Accessibility Update
* Webfont update

= 1.0.0 - Oct 19 2022 =
* Initial release

== Upgrade Notice ==

= 1.0.0 =
* Initial release

== Credits ==

* Based on Underscores https://underscores.me/, (C) 2012-2017 Automattic, Inc., [GPLv2 or later](https://www.gnu.org/licenses/gpl-2.0.html)

* normalize.css https://necolas.github.io/normalize.css/, (C) 2012-2016 Nicolas Gallagher and Jonathan Neal, [MIT](https://opensource.org/licenses/MIT)

* class-tgm-plugin-activation.php https://github.com/TGMPA/TGM-Plugin-Activation [GNU General Public License v2.0]

* SVG Icons function https://github.com/WordPress/twentyseventeen (C) 2016 WordPress.org, [GNU GPL, version 2 or later ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

* Radio-Image control https://github.com/reduxframework/kirki/ [MIT](https://opensource.org/licenses/MIT)

* Multi input custom control https://github.com/dingo-d/wordpress-theme-customizer-extra-custom-controls GNU General Public License v3.0 https://www.gnu.org/licenses/gpl-3.0.txt

* Customizer controls https://github.com/maddisondesigns/Customizer-Custom-Controls-Sample-Theme GNU General Public License v2 or later http://www.gnu.org/licenses/gpl-2.0.html

* Webfont loader https://github.com/WPTT/webfont-loader MIT License https://github.com/WPTT/webfont-loader/blob/master/LICENSE
